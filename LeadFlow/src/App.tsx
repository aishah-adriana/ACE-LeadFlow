...
const App = () => {
  ...
  const generateDraft = async (lead: Lead) => {
    ...
    try {
      ...
      setAiDraft("Failed to generate draft. Please check your API key.");
    } finally {
      setIsDrafting(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-8 text-slate-900 font-sans">
            ...
            <div className="p-8">
              {isDrafting ? (
                <div className="flex flex-col items-center py-12 gap-4">
                  ...
                  <p className="font-bold text-slate-400">Gemini is analyzing lead background...</p>
                </div>
              ) : (
      ...
      {isImporting && (
        <div className="max-w-[1600px] mx-auto mb-8 bg-white p-6 rounded-2xl border-2 border-blue-100 shadow-xl animate-in fade-in zoom-in duration-200">
          ...
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200 text-[10px] uppercase font-black text-slate-400 tracking-widest">
              <th className="p-4 w-12 text-center">
                <input type="checkbox" className="w-5 h-5 accent-blue-600" checked={selectedEmails.length === filteredLeads.length && filteredLeads.length > 0} onChange={() => setSelectedEmails(selectedEmails.length === filteredLeads.length ? [] : filteredLeads.map(l => l.email))} />
              </th>
              ...
              <tr key={l.email} className={`hover:bg-blue-50/20 transition-colors ${selectedEmails.includes(l.email) ? 'bg-blue-50/50' : ''}`}>
                ...
                <td className="p-4">
                  <div className="flex flex-wrap gap-1">
                    {l.program.split(', ').map(p => <span key={p} className="px-2 py-0.5 bg-slate-100 text-slate-500 rounded text-[9px] font-black border border-slate-200">{p}</span>)}
                  </div>
                </td>
                <td className="p-4 text-center">
